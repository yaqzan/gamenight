# Registers the "GameNight Watchdog" scheduled task (every 5 min).
...
$wdArgs = @()
if ($Port -ne 5004) { $wdArgs += @('-Port', $Port) }
if ($Tunnel -ne 'gamenight') { $wdArgs += @('-Tunnel', $Tunnel) }
...
try {
    ...
    Register-ScheduledTask -TaskName 'GameNight Watchdog' -Action $action `
        -Trigger $triggers -Settings $settings -Force -ErrorAction Stop | Out-Null
    Write-Host 'Registered task: GameNight Watchdog (every 5 min + at logon)'
} catch {
    Write-Host "Register-ScheduledTask failed ($($_.Exception.Message.Trim())) - falling back to schtasks"
    schtasks /create /tn "GameNight Watchdog" /sc minute /mo 5 /tr "wscript.exe $argString" /f
    if ($LASTEXITCODE -ne 0) { throw "schtasks fallback failed with exit code $LASTEXITCODE" }
    Write-Host 'Registered task: GameNight Watchdog (every 5 min, current user)'
}
