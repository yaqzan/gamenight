...
The app answers one question: "given who is at the table tonight, what should we play?" Every
rating serves that.
...
## What to research (in weight order)

1. **BGG**: weight rating + community player-count poll (Best/Recommended/Not Recommended per
   count). The strongest per-count signal.
2. **r/boardgames + BGG forums**: "how does X play at N" threads.
...
so. An honest error beats a made-up rating.

## Fields (exact vocab + limits: `gamenight/schema.py`)

- **pc**: only counts the game supports, each rated per the core rule. Wide party ranges may
  sample counts (2, 4, 6, 8...) the way Wavelength does.
